...
const hideControlsTemporarily = () => {
  // Hide only the primary floating scroll-top button; keep integrated nav buttons visible
  scrollTopBtn?.classList.remove('btn-visible');

  // Keep integrated nav buttons visible when nav is collapsed (they become compact icons).
  // If the nav is expanded and we want controls hidden, hide them.
...
const ensureBtn = (className, handler, labelText = '', iconChar = '') => {
  ...
  if (!btn) {
    ...
    // Structured content so CSS can hide/show label and show icon in collapsed state
    btn.innerHTML = `
      <span class="btn-char-icon" aria-hidden="true">${iconChar}</span>
    ...
    // ensure structured content present
    if (!btn.querySelector('.btn-char-icon') || !btn.querySelector('.btn-text-label')) {
      btn.innerHTML = `
        <span class="btn-char-icon" aria-hidden="true">${iconChar}</span>
        <span class="btn-text-label">${labelText}</span>
      `;
  ...
  const scrollToSection = (selector, fallbackText) => {
    ...
    if (target) {
      ...
      if (navMenuStack) {
         // Force default: collapsed by default (thin column)
  navMenuStack.classList.add('collapsed');
  document.body.classList.add('nav-collapsed');
  document.body.classList.remove('nav-visible');
  document.body.style.setProperty('--nav-current-width', getComputedStyle(document.documentElement).getPropertyValue('--nav-collapsed-width') || '50px');
